...
访问 http://localhost:1678，在「生成报告」页面输入：

> 湖北物理类考生，位次 4320，想学计算机、电子信息、自动化、电气工程，不想去偏远地区

即可得到一份包含冲/稳/保三档、录取概率、数据置信度、风险提示的完整 HTML 报告。
...
│   ├── data/raw_hubei_2024.csv    # 湖北演示数据
...
- Python 3.13+
- Node.js 20+
- Windows（`start.ps1` 为 PowerShell 脚本，macOS/Linux 可手动启动前后端）
...
| `/api/recommendations/from-text` | POST | 自由文本 → JSON 推荐 |
...
| 变量 | 说明 | 是否必填 |
|------|------|----------|
| `WINCODE_API_KEY` | WinCode / OpenAI 兼容 API Key | 否（不填则 LLM 增强关闭） |
